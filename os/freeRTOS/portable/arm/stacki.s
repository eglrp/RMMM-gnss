        ...
        AREA    gpOS_stack, CODE, READONLY

        IMPORT modeSVC_sp
        IMPORT modeIRQ_sp

        EXPORT vectors_init_stacks
...
vectors_init_stacks
    ...
    ;
        msr     cpsr_c, #MODE_UND:OR:I_BIT:OR:F_BIT    ; change to UNDEF mode with interrupts disabled
    ...
    ;
        msr     cpsr_c, #MODE_ABT:OR:I_BIT:OR:F_BIT    ; change to ABT mode with interrupts disabled
    ...
    ;
        msr     cpsr_c, #MODE_IRQ:OR:I_BIT:OR:F_BIT    ; change to IRQ mode with interrupts disabled
    ...
    ;
        msr     cpsr_c, #MODE_FIQ:OR:I_BIT:OR:F_BIT    ; change to FIQ mode with interrupts disabled
