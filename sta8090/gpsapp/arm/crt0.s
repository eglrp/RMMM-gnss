...
; crt0.s
;
; Implements hooks for ARM RVCT 3.1+ libraries for
...
;
        ...
        IMPORT  gpOS_memory_init                         ; from os20_memory.c
        IMPORT  gpOS_memory_partition_extend_heap
        IMPORT  gpOS_memory_protected
        ...
        IMPORT  os20_root_task_stack_size           ; from main.c
...
reset_handler
    ...
    ;
        msr     cpsr_c, #gpOS_MODE_SVC:OR:gpOS_I_BIT:OR:gpOS_F_BIT         ;// disable IRQs, move to SVC mode
    ...
    ;
    ; --- Execute specific OS20 bsp reset
    ;
...
__user_setup_stackheap
  ENDIF
    ...
    ;
        ...
        ldr     r4, =os20_root_task_stack_size
        ldr     r3, [r4]
        sub     sl, sp, r3
...
__os20_stackheapinit_stack_cycle
        str     r3, [r4]
        add     r4, r4, #0x4
        cmp     r4, sp
        bne     __os20_stackheapinit_stack_cycle
...
; --------------------------------------------------
    ;
    ; This APIs are called by ARM CRT to handle thread protection.
    ;

    ;
    ; This API is called by ARM CRT to init thread protection.
    ; It must return a non zero value to enable it.
    ;
        EXPORT  _mutex_initialize
_mutex_initialize
        b       gpOS_memory_protected

    ;
    ; This API is called by ARM CRT to lock memory handler.
    ;
        EXPORT  _mutex_acquire
_mutex_acquire
        b       gpOS_memory_lock

    ;
    ; This API is called by ARM CRT to unlock memory handler.
    ;
        EXPORT  _mutex_release
_mutex_release
        b       gpOS_memory_unlock
