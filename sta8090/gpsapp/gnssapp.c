...
#define NVM_VERSION_ID              (0x84814000U | NVM_VID_DR | NVM_VID_STAGPS | NVM_VID_COMPASS | NVM_VID_GALILEO | NVM_VID_GLONASS)
...
gpOS_error_t gnssapp_gnss_lib_start( gpOS_partition_t *gps_part, boolean_t dr_presence, tUInt sens_buf_size);
...
gpOS_error_t gnssapp_gnss_lib_start( gpOS_partition_t *gps_part, boolean_t dr_presence, tUInt sens_buf_size)
{
  ...
  // Initialize frontend default values if needed
  {
    ...
    for ( cnt = 0; cnt < 25; cnt++ )
    {
      ...
      sw_config_get_param( CURRENT_CONFIG_DATA, FE_A0_ID + (cnt * 2), &addr );
      sw_config_get_param( CURRENT_CONFIG_DATA, FE_D0_ID + (cnt * 2), &data );
  ...
  sw_config_get_param( CURRENT_CONFIG_DATA, PPS_CLOCK_SETTING_ID, &pps_clock );
...

  {
    ...
    sw_config_get_param( CURRENT_CONFIG_DATA, GPS_MIN_MAX_WEEK_NUMBER_ID, &min_week_n );
    ...
    sw_config_get_param( CURRENT_CONFIG_DATA, GPS_UTC_DEFAULT_SETTING_ID , &utc_delta_time_default );
  ...
  /**< Setup GPS engine */
  if( gnss_init_p( (gpOS_partition_t *)gps_part, orbit_list_selection, constellation_usage_selection, dr_presence, sens_buf_size) == GNSS_ERROR )
  {
  ...
  else
  {
    ...
    sw_config_get_param( CURRENT_CONFIG_DATA, GPS_MASK_ANGLE_ID, &mask_angle );
    sw_config_get_param( CURRENT_CONFIG_DATA, GPS_FIX_RATE_ID, &fix_rate );

    gnss_set_elevation_mask_angle( ( tDouble )mask_angle );
    gnss_set_fix_rate( fix_rate );

    sw_config_get_param( CURRENT_CONFIG_DATA, GPS_MASK_ANGLE_POSITIONING_ID, &mask_angle );
    gnss_set_elevation_mask_angle_positioning( ( tDouble )mask_angle );

    sw_config_get_param( CURRENT_CONFIG_DATA, WLS_CFG_PARAMS_1_ID, &wls_params);
...

    {
      ...
      sw_config_get_param( CURRENT_CONFIG_DATA, NOTCH_FILTER_CFG_ID, &notch_filter_cfg );
    ...
    else
    {
      tUInt dyn_params = 0;
      tUInt param;
      sw_config_get_param( CURRENT_CONFIG_DATA, DYNAMIC_MODE_CFG_PARAMS_ID, &dyn_params);
    ...
    {
      tUInt dsp_config_options = 0;
      sw_config_get_param( CURRENT_CONFIG_DATA, DSP_CONFIG_OPTIONS_ID, &dsp_config_options);
...

    {
      ...
      sw_config_get_param( CURRENT_CONFIG_DATA, LMS_ALGO_CFG_PARAMS_1_ID, &param1 );
      sw_config_get_param( CURRENT_CONFIG_DATA, LMS_ALGO_CFG_PARAMS_2_ID, &param2 );
    ...
    else
    {
      if(sw_config_get_software_switch_status( LOW_POWER_ON_OFF_SWITCH )!=FALSE)
      {
        ...
        sw_config_get_param( CURRENT_CONFIG_DATA, LOW_POWER_CFG_PARAMS_1_ID, &param1);
        sw_config_get_param( CURRENT_CONFIG_DATA, LOW_POWER_CFG_PARAMS_4_ID, &param4);
        ...
        cyclic.ms_off             = (tShort)((param1 >> 20U) & 0xFFFU);
  ...
  {
    tDouble pps_correction = 0.0, gps_pps_correction = 0.0, glonass_pps_correction = 0.0, compass_pps_correction = 0.0;
    ...
    sw_config_get_param( CURRENT_CONFIG_DATA, COMPASS_RF_TIME_CORRECTION_ID, &compass_pps_correction );
...

    {
      ...
      sw_config_get_param( CURRENT_CONFIG_DATA, HARDWARE_CONFIGURATION_ID, &hw_cfg );
...
#if defined(DR_DISABLE)
 /* In this case dr_plugin is diable; only starts the sensors*/
  {
    dr_sensors_config_t *dr_config_ptr;

    dr_config_ptr = (dr_sensors_config_t *)((tUInt)sw_config_private_get_base_address() + sizeof(tInt));

    if( dr_sensors_init_p( gps_part, dr_config_ptr) == GNSS_ERROR)
    {
...
gpOS_error_t gnssapp_nmea_start( gpOS_partition_t *part )
{
  ...
  {
    nmea_msg_list = ( (tInt)1 << GGA_NMEA_MSG ) | ( (tInt)1 << GSA_NMEA_MSG ) | ( (tInt)1 << GSV_NMEA_MSG ) | ( (tInt)1 << TG_NMEA_MSG ) | ( (tInt)1 << TS_NMEA_MSG ) | ( (tInt)1 << PA_NMEA_MSG ) | ( (tInt)1 << RMC_NMEA_MSG ) | ( (tInt)1 << RES_NMEA_MSG ) | ( (tInt)1 << WAAS_NMEA_MSG ); /* | DIFF_NMEA_MSG  | SBAS_NMEA_MSG; */
  }
...
static gpOS_error_t gnssapp_antenna_sensing_start( gpOS_partition_t *part )
{
  ...
  antenna_sensing_control_t crtl_mode;
  antenna_sensing_config_t cfg;
  tUInt switch_on_off_par;
  tBool switch_on_off;
  tUInt adc_antenna_sensing_ON_OFF;
  tUInt ante_sens_mode_off = (tUInt)ANTENNA_SENSING_MODE_OFF;
  ...
  adc_antenna_sensing_ON_OFF = (tUInt)(adc_antenna_sensing_cfg_params & 0x3U);

  switch_on_off_par = ( adc_antenna_sensing_cfg_params & 0x8U );
  ...
  if ( ante_sens_mode_off != ((tUInt)adc_antenna_sensing_ON_OFF & 0x3U) )
  {
    tU16 clk_div;
    tU32 as_adc_mode;

    clk_div = ( tU16 )( ( (tUInt)adc_antenna_sensing_cfg_params >> 4U ) & 0xFFU );
    as_adc_mode = (adc_antenna_sensing_ON_OFF & 0x3U);

    antenna_sensing_set_mode( ANTENNA_SENSING_MODE_ON );

    if( 0x1U == as_adc_mode )
    {
      crtl_mode = ANTENNA_SENSING_CONTROL_RF;
      antenna_sensing_set_switch( switch_on_off );
    }
    else if ( 0x2U == as_adc_mode )
    {
      ...
      crtl_mode = ANTENNA_SENSING_CONTROL_ADC;
    }
    else if ( 0x3U == as_adc_mode )
    {
      ...
      antenna_sensing_set_switch(switch_on_off);
      crtl_mode = ANTENNA_SENSING_CONTROL_GPIO;
    }/*lint !e9013: no 'else' at end of 'if ... else if' chain [MISRA 2012 Rule 15.7, required] */
...
gpOS_error_t gnssapp_init( gpOS_partition_t *fast_part )
{
  ...
  dr_presence = gnssapp_dr_init_presence( &sens_buf_size );

  gnssapp_startup_time.gnss_lib_start_cpu_time = gpOS_time_now();

  /**< Start GPS library */
  if( gnssapp_gnss_lib_start( fast_part,dr_presence,sens_buf_size) == gpOS_FAILURE)
  {
    return ( gpOS_FAILURE );
  }
  ...
  /*if ( modem_start( NULL ) == gpOS_FAILURE )
  {
    return ( gpOS_FAILURE );
  }*/
...
{
  return (tUInt)SW_CONFIG_VERSION_ID;
}
 ...
 * \return boolean_t dr_presence indicate if the Dr plugin is present
 *
 ***********************************************/
boolean_t gnssapp_dr_init_presence(tUInt *sens_buf_size)
{
#if defined DR_CODE_LINKED
  dr_sensors_config_t *dr_config_ptr;
...
#if !defined(DR_DISABLE)
  dr_initialize_Callbacks(sens_buf_size);
#endif /*DR_DISABLE*/
  return TRUE;
#else/*DR_CODE_LINKED */
  *sens_buf_size = 0;
